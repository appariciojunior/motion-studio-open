// Read MOVO's spinner scene DIRECTLY: the live camera and every card's world
// matrix, instead of inferring geometry from pixels.
//
...
// camera hides.
//
// Usage: node scripts/_scene_spinner.cjs "Spinner 01" "Hinge 01"
...
async function main() {
  ...
  const out = await page.evaluate(async (names, set) => {
    ...
    for (const name of names) {
      ...
      for (let s2 = 0; s2 < 4; s2++) {
        ...
        spins.push(first ? +(first.rotation.x + first.rotation.y).toFixed(5) : null);
      ...
      for (let p = n && n.parent; p; p = p.parent) chain.push({ type: p.type, rot: [p.rotation.x, p.rotation.y, p.rotation.z].map((v) => +v.toFixed(5)) });
      res.push({
        name,
    ...
    return res;
  }, wanted, process.env.MS_SET || null);

  fs.mkdirSync(OUT, { recursive: true });
  const file = path.join(OUT, 'movo-scene-spinner.json');
